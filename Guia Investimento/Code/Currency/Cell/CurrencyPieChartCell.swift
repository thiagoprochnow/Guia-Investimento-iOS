...
class CurrencyPieChartCell: UITableViewCell {
    ...
    override func layoutSubviews() {
        ...
        let currencies = currencyDB.getDataPercent()
        currencyDB.close()
        ...
        {
            if(index < 6){
                entries.append(PieChartDataEntry(value: currency.currentPercent/100, label: Utils.getCurrencyLabel(symbol: currency.symbol)))
            } else {
                // Check if it is last currency
                otherPercent += currency.currentPercent
