...
class StockPortfolioDB{
    ...
    func createTable(){
        let sql = "create table if not exists stock_portfolio (_id integer primary key autoincrement"
            + ", value_total real"
            + ", sold_total real"
    ...
    func save(_ portfolio: StockPortfolio){
        if(portfolio.id == 0){
            // Insert
            let sql = "insert or replace into stock_portfolio (value_total,sold_total,variation_total,income_total,value_gain,objective_percent,portfolio_percent,current_total,tax,brokerage,last_update) VALUES (?,?,?,?,?,?,?,?,?,?,?);"
            ...
            // Update
            let sql = "update stock_portfolio set value_total=?,sold_total=?,variation_total=?,income_total=?,value_gain=?,objective_percent=?,portfolio_percent=?,current_total=?,tax=?,brokerage=?,last_update=? where _id=?;"
    ...
    // Load StockPortfolio
    func getPortfolio() -> StockPortfolio {
        let portfolio = StockPortfolio()
        let stmt = db.query("SELECT * FROM stock_portfolio")
        if (db.nextRow(stmt)){
    ...
    func deleteById(_ id: Int){
        let sql = "delete from stock_portfolio where _id = ?"
