...
import Foundation
class FixedPortfolioDB{
    var db:SQLiteHelper
    init(){
        self.db = SQLiteHelper(database: "Portfolio.db")
    }
    
    // Create FixedPortfolio tables if it does not exists
    func createTable(){
        let sql = "create table if not exists fixed_portfolio (_id integer primary key autoincrement"
            + ", value_total real"
            + ", sold_total real"
    ...
    // Save or update a FixedPortfolio
    func save(_ portfolio: FixedPortfolio){
        if(portfolio.id == 0){
            // Insert
            let sql = "insert or replace into fixed_portfolio (value_total,sold_total,variation_total,income_total,value_gain,objective_percent,portfolio_percent,current_total,tax,brokerage,last_update) VALUES (?,?,?,?,?,?,?,?,?,?,?);"
            ...
            // Update
            let sql = "update fixed_portfolio set value_total=?,sold_total=?,variation_total=?,income_total=?,value_gain=?,objective_percent=?,portfolio_percent=?,current_total=?,tax=?,brokerage=?,last_update=? where _id=?;"
    ...
    // Load FixedPortfolio
    func getPortfolio() -> FixedPortfolio {
        let portfolio = FixedPortfolio()
        let stmt = db.query("SELECT * FROM fixed_portfolio")
        if (db.nextRow(stmt)){
    ...
    func deleteById(_ id: Int){
        let sql = "delete from fixed_portfolio where _id = ?"
