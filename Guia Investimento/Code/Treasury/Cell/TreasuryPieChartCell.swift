...
class TreasuryPieChartCell: UITableViewCell {
    ...
    override func layoutSubviews() {
        ...
        let treasuries = treasuryDB.getDataPercent()
        treasuryDB.close()
        ...
        {
            if(index < 6){
                entries.append(PieChartDataEntry(value: treasury.currentPercent/100, label: treasury.symbol))
            } else {
                // Check if it is last treasury
                otherPercent += treasury.currentPercent
