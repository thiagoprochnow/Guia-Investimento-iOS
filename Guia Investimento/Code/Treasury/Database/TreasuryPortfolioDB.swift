...
import Foundation
class TreasuryPortfolioDB{
    var db:SQLiteHelper
    init(){
        self.db = SQLiteHelper(database: "Portfolio.db")
    }
    
    // Create TreasuryPortfolio tables if it does not exists
    func createTable(){
        let sql = "create table if not exists treasury_portfolio (_id integer primary key autoincrement"
            + ", value_total real"
            + ", sold_total real"
    ...
    // Save or update a TreasuryPortfolio
    func save(_ portfolio: TreasuryPortfolio){
        if(portfolio.id == 0){
            // Insert
            let sql = "insert or replace into treasury_portfolio (value_total,sold_total,variation_total,income_total,value_gain,objective_percent,portfolio_percent,current_total,tax,brokerage,last_update) VALUES (?,?,?,?,?,?,?,?,?,?,?);"
            ...
            // Update
            let sql = "update treasury_portfolio set value_total=?,sold_total=?,variation_total=?,income_total=?,value_gain=?,objective_percent=?,portfolio_percent=?,current_total=?,tax=?,brokerage=?,last_update=? where _id=?;"
    ...
    // Load TreasuryPortfolio
    func getPortfolio() -> TreasuryPortfolio {
        let portfolio = TreasuryPortfolio()
        let stmt = db.query("SELECT * FROM treasury_portfolio")
        if (db.nextRow(stmt)){
    ...
    func deleteById(_ id: Int){
        let sql = "delete from treasury_portfolio where _id = ?"
