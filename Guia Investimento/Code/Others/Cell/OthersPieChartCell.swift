...
class OthersPieChartCell: UITableViewCell {
    ...
    override func layoutSubviews() {
        ...
        let otheres = othersDB.getDataPercent()
        othersDB.close()
        
        var entries: [PieChartDataEntry] = Array()
        
        var otherPercent = 0.0
        for (index,others) in otheres.enumerated()
        {
            if(index < 6){
                entries.append(PieChartDataEntry(value: others.currentPercent/100, label: others.symbol))
            } else {
                // Check if it is last others
                otherPercent += others.currentPercent
                if(index == (otheres.count - 1)){
