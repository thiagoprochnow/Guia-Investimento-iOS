...
import Foundation
class FiiPortfolioDB{
    var db:SQLiteHelper
    init(){
        self.db = SQLiteHelper(database: "Portfolio.db")
    }
    
    // Create FiiPortfolio tables if it does not exists
    func createTable(){
        let sql = "create table if not exists fii_portfolio (_id integer primary key autoincrement"
            + ", value_total real"
            + ", sold_total real"
    ...
    // Save or update a FiiPortfolio
    func save(_ portfolio: FiiPortfolio){
        if(portfolio.id == 0){
            // Insert
            let sql = "insert or replace into fii_portfolio (value_total,sold_total,variation_total,income_total,value_gain,objective_percent,portfolio_percent,current_total,tax,brokerage,last_update) VALUES (?,?,?,?,?,?,?,?,?,?,?);"
            ...
            // Update
            let sql = "update fii_portfolio set value_total=?,sold_total=?,variation_total=?,income_total=?,value_gain=?,objective_percent=?,portfolio_percent=?,current_total=?,tax=?,brokerage=?,last_update=? where _id=?;"
    ...
    // Load FiiPortfolio
    func getPortfolio() -> FiiPortfolio {
        let portfolio = FiiPortfolio()
        let stmt = db.query("SELECT * FROM fii_portfolio")
        if (db.nextRow(stmt)){
    ...
    func deleteById(_ id: Int){
        let sql = "delete from fii_portfolio where _id = ?"
