...
class FiiPortfolioView: UIViewController, UITableViewDataSource, UITableViewDelegate {
    @IBOutlet var fab: UIImageView!
    @IBOutlet var bgView: UIView!
    @IBOutlet var tableView: UITableView!
    @IBOutlet var emptyListView: UILabel!
    var portfolio: FiiPortfolio!
    ...
    override func viewDidLoad() {
        ...
        let xibPortfolio = UINib(nibName: "FiiPortfolioCell", bundle: nil)
        let xibPie = UINib(nibName: "FiiPieChartCell", bundle: nil)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(linha == 0){
            // Load Fii Portfolio information on cell
            let cell = self.tableView.dequeueReusableCell(withIdentifier: "cellPortfolio") as! FiiPortfolioCell
            // Do not show highlight when selected
            cell.selectionStyle = .none
            let locale = Locale(identifier: "pt_BR")
            let buyTotal = portfolio.buyTotal
            let fiiAppreciationPercent = "(" + String(format: "%.2f", locale: locale, arguments: [portfolio.variationTotal/buyTotal * 100]) + "%)"
            ...
            cell.variationPercent.text = fiiAppreciationPercent
            ...
            let cell = self.tableView.dequeueReusableCell(withIdentifier: "cellPie") as! FiiPieChartCell
            // Do not show highlight when selected
            cell.selectionStyle = .none
            return cell
        } else {
